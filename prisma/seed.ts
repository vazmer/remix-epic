...
async function seed() {
	...
	console.time(`🐨 Created admin user "vazmer"`)

	const images = await promiseHash({
		user: img({ filepath: './tests/fixtures/images/user/kody.png' }),
		cuteKoala: img({
	...
	await prisma.user.create({
		...
		data: {
			...
			username: 'vazmer',
			name: 'Bojan Vazmer',
			image: { create: images.user },
			password: { create: createPassword('81z6IEsK') },
			connections: {
				create: { providerName: 'github', providerId: githubUser.profile.id },
			},
			roles: {
				connect: [{ name: 'admin', description: '' }, { name: 'user' }],
			},
			notes: {
				create: [
					{
						...
						images: {
							create: [images.cuteKoala, images.koalaEating],
						},
					},
					{
						id: '414f0c09',
	...
	console.timeEnd(`🐨 Created admin user "vazmer"`)
